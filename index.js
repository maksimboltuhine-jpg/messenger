...
const { ExpressPeerServer } = require('peer'); // Подключаем звонки
...
console.log('🚀 v13.0: CHAT & CALLS ONLINE');
...
user = new User({ login, password: hashPassword, uid: `${uid}` }); // UID без решетки для PeerJS
await user.save();
} else {
...
await Msg.findByIdAndDelete(id);if (fileId && gfsBucket) gfsBucket.delete(new mongoose.Types.ObjectId(fileId)).catch(() => {});
